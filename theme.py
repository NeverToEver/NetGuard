...
def build_colors(dark: bool) -> dict[str, str]:
    return {
        "bg": "#0b1117" if dark else "#eef2f5",
        "toolbar": "#111923" if dark else "#dde6ef",
        "filter": "#172231" if dark else "#d7ebf8",
        "summary": "#0f1722" if dark else "#f8fafc",
        "panel": "#151e2a" if dark else "#ffffff",
        "panel_alt": "#101923" if dark else "#f3f7fb",
        "field": "#0d141d" if dark else "#ffffff",
        "field_alt": "#162131" if dark else "#ecf2f8",
        "text": "#f3f7fb" if dark else "#102033",
        "muted": "#a6b4c3" if dark else "#536475",
        "accent": "#2f8cc8" if dark else "#0067a0",
        "accent_active": "#4aa3dd" if dark else "#005484",
        "accent_text": "#ffffff",
        ...
        "select": "#1d5f91" if dark else "#b8dffc",
        "button": "#202b38" if dark else "#edf3f8",
        "button_active": "#2a3747" if dark else "#dce8f1",
        "border": "#2e3a48" if dark else "#b7c6d3",
        "focus": "#79c7ff" if dark else "#0067a0",
    }
...
class ThemeManager:
    def __init__(self, app: tk.Tk) -> None:
        ...
        self._font_heading.configure(family=cjk, size=12, weight="bold")
    ...
    def apply(self, dark: bool, table: ttk.Treeview, classic_widgets: list[tk.Widget]) -> None:
        ...
        style.configure("Treeview", background=colors["field"], foreground=colors["text"],
                        fieldbackground=colors["field"], bordercolor=colors["border"], rowheight=23, font=self._font_body)
